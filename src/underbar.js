(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(collectiveResult, item){
  ...
  _.extend = function(obj) {
    _.each(arguments, function(eachObj){
      _.each(eachObj, function(item, key){
        obj[key] = item;
      });
    });
    return obj;
  ...
  _.shuffle = function(array) {
    ...
    for (let i = arr.length-1; i>=0; i--){
      var chosenSpot = Math.floor(Math.random()*i)
      var chosenSpotsValue = arr[chosenSpot]
      arr[chosenSpot] = arr[i];
      arr[i] = chosenSpotsValue
    }
    return arr;
  };
